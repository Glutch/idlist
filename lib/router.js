...
Router.route('/', function () {
  ...
  if (this.ready()) {
    if(Tasks.find({username: Meteor.user().username}).count() == 0 && Projects.find({username: Meteor.user().username}).count() == 0){
      ...
      Meteor.call('tasks.insert', 'To view your completed tasks, press the green button.', project)

      setTimeout(function(){
        Meteor.call('tasks.insert', 'To check a task as completed, click the white circle.', project)
        setTimeout(function(){
          Meteor.call('tasks.insert', 'You can create a new project / todolist by clicking on the blue button', project)
          setTimeout(function(){
